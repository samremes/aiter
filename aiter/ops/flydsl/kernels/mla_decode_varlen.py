...
V3_KV_BYTES_PER_SUB_BLOCK: int = (
    V3_KV_SUB_BLOCK_ROWS * V3_KV_SUB_BLOCK_COLS
)  # 128 (fp8)
...
V3_KV_BYTES_PER_BLOCK: int = (
    V3_KV_BYTES_PER_2SUB_PADDED * V3_KV_NUM_2SUBS
)  # 264 * 16 = 4224
...
V3_KV_NUM_WARPS_PER_COL: int = (
    NUM_WARPS // V3_KV_NUM_COL_STRIPS
)  # 4 warps per col-strip per pass
V3_KV_ROW_PASS_SLOT_STRIDE: int = (
    V3_KV_NUM_WARPS_PER_COL * 2
)  # 8 paired-slots between pass 0 and pass 1
...
assert (
    max(SZ_LDS_O16, SZ_LDS_O32) <= V3_SZ_LDS_KV
), "Output LDS must fit in one gfx950 KV buffer region"
...
def kn_mla_fwd_decode_m16x8_fp8_fp8(
    ...
    num_heads: fx.Constexpr,
    direct_output: fx.Constexpr,
):
    ...
    def _mfma_fp8_bf8(result_type, operands, **kw):
        return rocdl.mfma_f32_16x16x32_fp8_bf8(
            ...
            operands[3],
            operands[4],
            rocdl._blgp_attr(operands[5]),
            **kw,
        ).result

    def _unpack_fp8x8(packed):
        packed_value = ArithValue(packed)
        words = [fx.Int32(_raw(packed_value)), fx.Int32(_raw(packed_value >> 32))]
        values = []
        for word_index in range_constexpr(2):
            low = Vec(rocdl.cvt_pk_f32_fp8(T.f32x2, words[word_index], False))
            high = Vec(rocdl.cvt_pk_f32_fp8(T.f32x2, words[word_index], True))
            values.extend([low[0], low[1], high[0], high[1]])
        return values
    ...
    # ---- Constants ----
    c_neg_inf = fx.Float32(-1.0e30)
    c_zero_f32 = fx.Float32(0.0)
    c_one_f32 = fx.Float32(1.0)
    c_p_scale = fx.Float32(256.0)
    c_p_scale_recip = fx.Float32(1.0 / 256.0)
    ...
    scale_qk = softmax_scale * q_scale_value * kv_scale_value
    if const_expr(direct_output):
        output_rsrc = buffer_ops.create_buffer_resource(final_output)
    else:
        output_rsrc = buffer_ops.create_buffer_resource(split_output)
    ...
    heads_per_cta = min(num_heads, 64)
    head_groups = heads_per_cta // TILE_M
    value_groups = NUM_WARPS // head_groups
    local_v_head_dim = V_HEAD_DIM // value_groups
    local_num_pv_iters = NUM_PV_ITERS // value_groups
    logical_head_group = warp_idx / value_groups
    logical_value_group = warp_idx % value_groups
    value_col_offset = logical_value_group * local_v_head_dim

    # ---- Work range ----
    work_range = buffer_ops.buffer_load(
        buffer_ops.create_buffer_resource(work_indptr),
        worker_idx * 2,
        vec_width=2,
        dtype=T.i32,
    )
    ...
    if const_expr(IS_GFX950):
        ...
        kv_ld_row_base = (
            (warp_idx % V3_KV_NUM_WARPS_PER_COL) * V3_KV_SUB_BLOCK_ROWS
            + (lane_idx / 32) * 16
            + (lane_idx % 32) / 8
        )
        kv_ld_col_base = _i32(
            (warp_idx / V3_KV_NUM_WARPS_PER_COL) * V3_KV_SUB_BLOCK_COLS
            + (lane_idx % 8) * 4
        )
    else:
    ...
    def _async_load_k_tile(
        ...
        if const_expr(check_boundary):
            ...
            if is_oob:
                ...
                fx.ptr_store(
                    Vec.from_elements([c_zero_i32], fx.Int32).bitcast(fx.Uint8),
                    _lds_ptr(lds_addr),
                )
            else:
                _emit_vram_to_lds()
    ...
    def _prefetch_k_tile_asm(
        ...
        if const_expr(IS_GFX950):
            ...
            lds_adjust = (
                block_idx_const * KV_BLOCK_BYTES - block_idx_const * KV_NUM_COLS
            )
        lds_base_i32 = _i32(ArithValue(p_lds_kv_warp) + lds_adjust)
        ...
        if const_expr(check_boundary is False):
            ...
            if is_oob:
                ...
                _inline_asm_void(
                    [lds_zero_addr, _raw(c_zero_i32)], "ds_write_b32 $0, $1", "v,v"
                )
            else:
                _emit_normal_load()
    ...
    def _load_v_from_lds(p_lds_kv_base_idx, warp_idx_val, lane_idx_val):
        ...
        for pass_idx in range_constexpr(4):
            ...
            data_vec = fx.ptr_load(
                _lds_ptr(lds_addr, extra_bytes=off),
                result_type=fx.Vector.make_type(8, fx.Uint8),
            ).bitcast(fx.Int32)
    ...
    def _store_vt_to_lds(vt_lds_base_idx, warp_idx_val, lane_idx_val, vt8):
        ...
        fx.ptr_store(
            hi_packed.bitcast(fx.Uint8), _lds_ptr(_i32(ArithValue(lds_vt_addr) + 16))
        )
    ...
    def _warp_reduce_max_16(val):
        ...
        for sh in [32, 16]:
            ...
            w = fx.maxnumf(
                w, _shfl_xor_f32(w, sh), fastmath=arith.FastMathFlags.contract
            )
        return w
    ...
    def _load_q_to_regs(qo_start_i32, query_head_group):
        ...
        s_offset = query_head_group * (16 * QK_HEAD_DIM) + _idx(qo_start_i32) * (
        ...
        lds_st_offset = (
            (row_st / 2) * Q_BYTES_PER_2ROWS + (row_st % 2) * Q_ELEM_PER_ROW + col_st
        )
        ...
        lds_ld_offset = (
            (row_ld / 2) * Q_BYTES_PER_2ROWS + (row_ld % 2) * Q_ELEM_PER_ROW + col_ld
        )
        ...
        def _shuffle_q_through_lds(q_vram_data):
            ...
            q0 = _raw(
                fx.ptr_load(
                    _lds_ptr(lds_rd_addr), result_type=fx.Vector.make_type(8, fx.Uint8)
                ).bitcast(fx.Int64)[0]
            )
            q1 = _raw(
                fx.ptr_load(
                    _lds_ptr(lds_rd_addr, extra_bytes=MFMA_K),
                    result_type=fx.Vector.make_type(8, fx.Uint8),
                ).bitcast(fx.Int64)[0]
            )
            return (q0, q1)
    ...
    def _softmax_scale_p(p_vals, col_0_start, kv_end_i32, check_boundary):
        ...
        for i in range_constexpr(P_VALS_PER_THR):
            result[i] = _f32(p_vals[i]) * scale_qk
    ...
    def _softmax(
        ...
        for i in range_constexpr(1, P_VALS_PER_THR):
            local_max = fx.maxnumf(
                local_max, scaled[i], fastmath=arith.FastMathFlags.contract
            )
        ...
        if const_expr(is_first_iter):
            ...
            new_row_max = fx.maxnumf(
                local_max, row_max_old, fastmath=arith.FastMathFlags.contract
            )
    ...
    def _pack_p_to_fp8(p_exp_vals):
        """Pack scaled P to E4M3 plus an E5M2 residual for PV MFMA."""

        def _pack8(v):
            scaled = [_f32(value) * c_p_scale for value in v]
            high_word0 = rocdl.cvt_pk_fp8_f32(
                T.i32, scaled[0], scaled[1], c_zero_i32, 0
            )
            high_word0 = rocdl.cvt_pk_fp8_f32(
                T.i32, scaled[2], scaled[3], high_word0, 1
            )
            high_word1 = rocdl.cvt_pk_fp8_f32(
                T.i32, scaled[4], scaled[5], c_zero_i32, 0
            )
            high_word1 = rocdl.cvt_pk_fp8_f32(
                T.i32, scaled[6], scaled[7], high_word1, 1
            )
            high = _pack_i32x2(high_word0, high_word1)
            rounded = _unpack_fp8x8(high)
            residual = [scaled[i] - rounded[i] for i in range_constexpr(8)]
            residual_word0 = rocdl.cvt_pk_bf8_f32(
                T.i32,
                _raw(residual[0]),
                _raw(residual[1]),
                _raw(c_zero_i32),
            ...
            residual_word1 = rocdl.cvt_pk_bf8_f32(
                T.i32,
                _raw(residual[4]),
                _raw(residual[5]),
                _raw(c_zero_i32),
            ...
            return high, _pack_i32x2(residual_word0, residual_word1)

        if const_expr(IS_GFX950):
            high_lo, residual_lo = _pack8(p_exp_vals[0:8])
            high_hi, residual_hi = _pack8(p_exp_vals[8:16])
            return high_lo, high_hi, residual_lo, residual_hi
    ...
    def _rescale_oaccu(oaccu, rescale):
        ...
        for i in range_constexpr(local_num_pv_iters * 2):
    ...
    def _process_tile_gemm1(
        ...
        for rope_pair in range_constexpr(NUM_ROPE_ITERS):
            ...
            k0 = [
                _load_k_from_lds(k_base_i32, 16 * h, (tile_0 + 16) * BLOCK_K)
                for h in range_constexpr(P_COMP_SUBS)
            ]
            k1 = [
                _load_k_from_lds(k_base_i32, 16 * h, (tile_1 + 16) * BLOCK_K)
                for h in range_constexpr(P_COMP_SUBS)
            ]
            ...
            for h in range_constexpr(P_COMP_SUBS):
                p_comp[h] = _mfma_fp8(
                    T.f32x4, [k0[h], q_rope[tile_0], p_comp[h], 0, 0, 0]
                )
            ...
            for h in range_constexpr(P_COMP_SUBS):
                p_comp[h] = _mfma_fp8(
                    T.f32x4, [k1[h], q_rope[tile_1], p_comp[h], 0, 0, 0]
                )
        ...
        if const_expr(IS_GFX950):
            if const_expr(do_resolve_nn is not None):
                ...
                if do_resolve_nn:
                    row_kv_ld_nn_p0 = _get_kv_ld_row(
                        nn_resolve_start, nn_resolve_end, True, pass_idx=0
                    )
                    row_kv_ld_nn_p1 = _get_kv_ld_row(
                        nn_resolve_start, nn_resolve_end, True, pass_idx=1
                    )
            else:
                row_kv_ld_nn_p0 = fx.Int32(-1)
                row_kv_ld_nn_p1 = fx.Int32(-1)
            ...
            if const_expr(do_resolve_nn is not None):
                ...
                if do_resolve_nn:
                    row_kv_ld_nn = _get_kv_ld_row(
                        nn_resolve_start, nn_resolve_end, True
                    )
            else:
                row_kv_ld_nn = fx.Int32(-1)
        ...
        if const_expr(IS_GFX950):
            ...
            p_pack_lo, p_pack_hi, p_residual_lo, p_residual_hi = p_pack
            return (
                row_max_new,
                row_sum_e_new,
                p_pack_lo,
    ...
    def _gemm2_core(p_pack, oaccu, vt_base_i32):
        ...
        K_HALVES = BLOCK_N // 32
        rocdl.s_setprio(15)
        for pv_pair in range_constexpr(local_num_pv_iters // 2):
            iter_a = pv_pair * 2
            iter_b = pv_pair * 2 + 1
            ...
            if const_expr(K_HALVES == 2):
                p_high, p_residual = p_pack
                p_lo, p_hi = p_high
                p_residual_lo, p_residual_hi = p_residual
            ...
            for step in range_constexpr(K_HALVES * 2):
                ...
                p_arg = p_args[step]
                p_residual_arg = p_residual_args[step]
                acc_idx = iter_idx * 2
                high0 = _mfma_fp8(
                    T.f32x4,
                    [lhs0, p_arg, oaccu[acc_idx], 0, 0, 0],
                )
                high1 = _mfma_fp8(
                    T.f32x4,
                    [lhs1, p_arg, oaccu[acc_idx + 1], 0, 0, 0],
                )
                oaccu[acc_idx] = _mfma_fp8_bf8(
                    T.f32x4,
                    [lhs0, p_residual_arg, high0, 0, 0, 0],
                )
                oaccu[acc_idx + 1] = _mfma_fp8_bf8(
                    T.f32x4,
                    [lhs1, p_residual_arg, high1, 0, 0, 0],
                )

            rocdl.sched_barrier(0)

            if const_expr(pv_pair < local_num_pv_iters // 2 - 1):
                rocdl.s_nop(1)
    ...
    def _gemm2_first_iter(p_pack, vt_base_i32):
        ...
        _barrier(lgkmcnt=0)
        rocdl.sched_barrier(0)
        oaccu = [c_zero_v4f32] * (local_num_pv_iters * 2)
    ...
    def _pack_f32x4_to_bf16_2dw(acc_val):
        ...
        values = Vec(acc_val)
        return (
            rocdl.cvt_pk_bf16_f32(values[0], values[1]),
            rocdl.cvt_pk_bf16_f32(values[2], values[3]),
        )
    ...
    def _store_oaccu_pair_bf16(oaccu_a, oaccu_b, tile_idx, p_lds_o, row_base_i32):
        ...
        data = _raw(
            fx.ptr_load(
                _lds_ptr(lds_rd_addr), result_type=fx.Vector.make_type(16, fx.Uint8)
            ).bitcast(fx.Int32)
        )
        ...
        col_vram = ArithValue(o16_col_ld) + value_col_offset + tile_idx * MFMA_N * 2
        vram_offset = _raw((row_vram * V_HEAD_DIM + col_vram) * 2)
        buffer_ops.buffer_store(data, output_rsrc, vram_offset, offset_is_bytes=True)
    ...
    def _store_oaccu_pair_split(oaccu_a, oaccu_b, tile_idx, p_lds_o, row_base_i32):
        ...
        col_offset_i32 = value_col_offset + tile_idx * MFMA_N * 2
        ...
        data_1 = _raw(
            fx.ptr_load(
                _lds_ptr(lds_rd_addr, extra_bytes=O32_LD_DELTA),
        ...
        buffer_ops.buffer_store(
            _raw(Vec(data_0).bitcast(fx.Int32)),
            output_rsrc,
            vram_off_0,
        ...
        buffer_ops.buffer_store(
            _raw(Vec(data_1).bitcast(fx.Int32)),
            output_rsrc,
            vram_off_1,
            offset_is_bytes=True,
        )
    ...
    def _gemm2_last_with_store(
        ...
        reci_vec = _raw(
            Vec.filled(4, fx.Float32(reci_sum) * c_p_scale_recip, fx.Float32)
        )
        ...
        for pv_pair in range_constexpr(local_num_pv_iters // 2):
            ...
            for step in range_constexpr(K_HALVES * 2):
                ...
                high0 = _mfma_fp8(
                    T.f32x4,
                    [lhs0, p_arg, oaccu_in[acc_idx], 0, 0, 0],
                )
                high1 = _mfma_fp8(
                    T.f32x4,
                    [lhs1, p_arg, oaccu_in[acc_idx + 1], 0, 0, 0],
                )
                acc0 = _mfma_fp8_bf8(
                    T.f32x4,
                    [lhs0, p_residual_arg, high0, 0, 0, 0],
    ...
    def _vt_base_i32():
        ...
        vt_block_offset = (
            vt_row_blk * VT_BLKS_PER_ROW_PAD + vt_col_blk
        ) * VT_ELEMS_PER_BLK
    ...
    if const_expr(IS_GFX950):
        ...
        def _v_base_i32(p_lds_kv_base):
            ...
            """
            value_group_offset = (
                value_col_offset // KV_NUM_COLS
            ) * V3_KV_BYTES_PER_BLOCK
            return _i32(
                ArithValue(p_lds_kv_base) + v_lds_lane_offset + value_group_offset
            )
    ...
    for work_idx in range(work_start_idx, work_end_idx):
        ...
        head_tile = rocdl.readfirstlane(T.i32, wi_dw5_6_vec[1])
        query_head_group = _idx(head_tile) * (64 // TILE_M) + logical_head_group
        ...
        if first_tile_needs_boundary:
            ...
            row_kv_ld_first_p0 = _get_kv_ld_row(
                kv_start, kv_first_end, False, pass_idx=0
            )
            if const_expr(IS_GFX950):
                row_kv_ld_first_p1 = _get_kv_ld_row(
                    kv_start, kv_first_end, False, pass_idx=1
                )

        # Load Q to GPR (independent of boundary check)
        q_nope_packs, q_rope_packs = _load_q_to_regs(qo_start, query_head_group)
        ...
        if tile1_is_full:
            row_kv_ld_tile1_p0 = _get_kv_ld_row(
                kv_start_plus_bn, kv_start_plus_2bn, False, pass_idx=0
            )
            if const_expr(IS_GFX950):
                row_kv_ld_tile1_p1 = _get_kv_ld_row(
                    kv_start_plus_bn, kv_start_plus_2bn, False, pass_idx=1
                )
        else:
            row_kv_ld_tile1_p0 = _get_kv_ld_row(
                kv_start_plus_bn, _raw(kv_end), True, pass_idx=0
            )
            if const_expr(IS_GFX950):
                row_kv_ld_tile1_p1 = _get_kv_ld_row(
                    kv_start_plus_bn, _raw(kv_end), True, pass_idx=1
                )
        ...
        p_residual_first_lo = fx.Int64(0)
        p_residual_first_hi = fx.Int64(0)
        ...
        if const_expr(IS_GFX950):
            p_pack_first = (
                (p_pack_first_lo, p_pack_first_hi),
                (p_residual_first_lo, p_residual_first_hi),
            )
        else:
        ...
        def _write_lse(pqo_loc_i32, rm, rse):
            ...
            store_lse = (ArithValue(logical_value_group) == 0) & (
                ArithValue(lane_idx) < 16
            )
            if store_lse:
        ...
        def _do_last_gemm2_and_store(
            ...
            """
            reci = fx.Float32(rocdl.rcp(T.f32, rse))
            if const_expr(direct_output):
                reci = reci * kv_scale_value
                row_base = _raw(_idx(qo_start) * num_heads + query_head_group * 16)
            else:
                row_base = _raw(
                    _idx(partial_qo_loc) * num_heads + query_head_group * 16
                )
        ...
        def _multi_tile_path():
            ...
            for tile_iv, state in range(
                _idx(1), _idx(num_tiles_m1), _idx(1), init=init_args
            ):
                ...
                rm_carried = state[0]
                rse_carried = state[1]
                oaccu_carried = [state[2 + i] for i in range(local_num_pv_iters * 2)]
                row_kv_ld_next_arg = (
                    state[2 + local_num_pv_iters * 2],
                    state[2 + local_num_pv_iters * 2 + 1],
                )

                # Buffer parity
                is_odd = (tile_iv_i32 & 1) != 0
                curr_base_idx = ArithValue(is_odd).select(
                    p_lds_kv_1_base, p_lds_kv_0_base
                )
                ...
                pp_m_lo = fx.Int64(0)
                pp_m_hi = fx.Int64(0)
                pp_m_residual_lo = fx.Int64(0)
                pp_m_residual_hi = fx.Int64(0)
                nn_m_p0 = fx.Int32(-1)
                nn_m_p1 = fx.Int32(-1)
                if mid_cbn:
                ...
                if const_expr(IS_GFX950):
                    pp_m = (
                        (pp_m_lo, pp_m_hi),
                        (pp_m_residual_lo, pp_m_residual_hi),
                    )
                    oa_m = _gemm2_with_rescale(
                        pp_m, rs_m, oaccu_carried, _v_base_i32(curr_base_idx)
                    )
                else:
                    oa_m = _gemm2_with_rescale(
                        pp_m_lo, rs_m, oaccu_carried, _vt_base_i32()
                    )
            ...
            oaccu_mt = [results[2 + i] for i in range(local_num_pv_iters * 2)]
            ...
            last_curr_base = ArithValue(last_is_odd).select(
                p_lds_kv_1_base, p_lds_kv_0_base
            )
            ...
            last_o_base = ArithValue(last_is_odd).select(
                p_lds_kv_0_base, p_lds_kv_1_base
            )
            ...
            if const_expr(IS_GFX950):
                pp_l = (
                    (pp_l_lo, pp_l_hi),
                    (pp_l_residual_lo, pp_l_residual_hi),
                )
            else:
                pp_l = pp_l_lo
        ...
        def _single_tile_path():
            ...
            oaccu_st = [c_zero_v4f32] * (local_num_pv_iters * 2)
            _do_last_gemm2_and_store(
                p_pack_first,
                rescale_first,
                oaccu_st,
...
def kn_build_varlen_work_info(
    ...
    num_splits: fx.Constexpr,
    head_ctas: fx.Constexpr,
):
    item = gpu.block_idx.x * 256 + gpu.thread_id("x")
    total = max_rows * num_splits * head_ctas
    if item < total:
        item_i32 = fx.Int32(item)
        base_item = item_i32 // fx.Int32(head_ctas)
        head_tile = item_i32 % fx.Int32(head_ctas)
        row = base_item // fx.Int32(num_splits)
        split = base_item % fx.Int32(num_splits)
        ...
        row_start = fx.Int32(
            buffer_ops.buffer_load(indptr_rsrc, row, vec_width=1, dtype=T.i32)
        )
        row_end = fx.Int32(
            buffer_ops.buffer_load(
                indptr_rsrc, row + fx.Int32(1), vec_width=1, dtype=T.i32
            )
        )
        row_len = row_end - row_start
        split_len = (row_len + fx.Int32(num_splits - 1)) // fx.Int32(num_splits)
        kv_start = row_start + split * split_len
        kv_end_unclamped = kv_start + split_len
        kv_end = (kv_end_unclamped > row_end).select(row_end, kv_end_unclamped)
        active = (row < live_rows) & (kv_start < kv_end)
        work_start = active.select(item_i32, fx.Int32(0))
        work_end = active.select(item_i32 + fx.Int32(1), fx.Int32(0))
        buffer_ops.buffer_store(work_start, work_indptr_rsrc, item * 2)
        buffer_ops.buffer_store(work_end, work_indptr_rsrc, item * 2 + 1)
        base = item * SIZE_MLA_WORK_INFO_IN_DW
        buffer_ops.buffer_store(fx.Int32(0), info_rsrc, base)
        buffer_ops.buffer_store(base_item, info_rsrc, base + 1)
        buffer_ops.buffer_store(row, info_rsrc, base + 2)
        buffer_ops.buffer_store(row + fx.Int32(1), info_rsrc, base + 3)
        buffer_ops.buffer_store(kv_start, info_rsrc, base + 4)
        buffer_ops.buffer_store(kv_end, info_rsrc, base + 5)
        buffer_ops.buffer_store(head_tile, info_rsrc, base + 6)
        buffer_ops.buffer_store(fx.Int32(0), info_rsrc, base + 7)


@flyc.jit
def launch_build_varlen_work_info(
    query_start_loc: fx.Tensor,
    kv_indptr: fx.Tensor,
    work_indptr: fx.Tensor,
    work_info_set: fx.Tensor,
...
def kn_zero_varlen_empty_rows(
    ...
    if row < live_rows:
        ...
        row_end = fx.Int32(
            buffer_ops.buffer_load(indptr_rsrc, row + 1, vec_width=1, dtype=T.i32)
        )
        if row_start == row_end:
            output_base = (row * num_heads + head) * V_HEAD_DIM + tid * 2
            zeros = Vec.from_elements([fx.BFloat16(0.0), fx.BFloat16(0.0)], fx.BFloat16)
            buffer_ops.buffer_store(zeros, output_rsrc, output_base)


@flyc.jit
def launch_zero_varlen_empty_rows(
    query_start_loc: fx.Tensor,
    kv_indptr: fx.Tensor,
    final_output: fx.Tensor,
    num_requests: fx.Int32,
...
def kn_merge_varlen_partials(
    ...
    output_rsrc = buffer_ops.create_buffer_resource(final_output)
    live_rows = buffer_ops.buffer_load(qsl_rsrc, num_requests, vec_width=1, dtype=T.i32)
    if row < live_rows:
        row_start = buffer_ops.buffer_load(indptr_rsrc, row, vec_width=1, dtype=T.i32)
        row_end = buffer_ops.buffer_load(indptr_rsrc, row + 1, vec_width=1, dtype=T.i32)
        output_base = (row * num_heads + head) * V_HEAD_DIM + tid * 2
        if row_start == row_end:
            zeros = Vec.from_elements([fx.BFloat16(0.0), fx.BFloat16(0.0)], fx.BFloat16)
            buffer_ops.buffer_store(zeros, output_rsrc, output_base)
            ...
            row_len = row_end - row_start
            split_len = (row_len + num_splits - 1) // num_splits
            maximum = fx.Float32(float("-inf"))
            denominator = fx.Float32(0.0)
            accumulator0 = fx.Float32(0.0)
            accumulator1 = fx.Float32(0.0)
            for split in range_constexpr(num_splits):
                split_start = row_start + split * split_len
                if split_start < row_end:
                    item = row * num_splits + split
                    lse_index = item * num_heads + head
                    lse = fx.Float32(
                        buffer_ops.buffer_load(
                            lse_rsrc, lse_index, vec_width=1, dtype=T.f32
                        )
                    )
                    next_maximum = fx.maxnumf(maximum, lse)
                    old_weight = rocdl.exp2(
                        T.f32, _raw((maximum - next_maximum) * LOG2E)
                    )
                    new_weight = rocdl.exp2(T.f32, _raw((lse - next_maximum) * LOG2E))
                    partial_base = (item * num_heads + head) * V_HEAD_DIM + tid * 2
                    values = Vec(
                        buffer_ops.buffer_load(
                            partial_rsrc, partial_base, vec_width=2, dtype=T.f32
                        )
                    )
                    accumulator0 = accumulator0 * fx.Float32(old_weight) + fx.Float32(
                        values[0]
                    ) * fx.Float32(new_weight)
                    accumulator1 = accumulator1 * fx.Float32(old_weight) + fx.Float32(
                        values[1]
                    ) * fx.Float32(new_weight)
                    denominator = denominator * fx.Float32(old_weight) + fx.Float32(
                        new_weight
                    )
                    maximum = next_maximum
            scale = fx.Float32(
                buffer_ops.buffer_load(scale_rsrc, 0, vec_width=1, dtype=T.f32)
            )
            result = Vec.from_elements(
                [
                    fx.BFloat16(accumulator0 / denominator * scale),
                    fx.BFloat16(accumulator1 / denominator * scale),
                ],
                fx.BFloat16,
            )
            buffer_ops.buffer_store(result, output_rsrc, output_base)


@flyc.jit
def launch_merge_varlen_partials(
    query_start_loc: fx.Tensor,
    kv_indptr: fx.Tensor,
    partial_output: fx.Tensor,
    partial_lse: fx.Tensor,
